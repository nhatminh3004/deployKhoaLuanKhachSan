...
import { checkPhoneExistRoute, forgotPassWord, registerRoute } from "../utils/APIRoutes";
...
function ForgotPassword() {
  ...
  const handleSendCode = async (e) => {
    ...
    if(soDienThoai.length===0){
      ...
      console.log('Res Forgot Password:',res.data);
      if(res.data===true){
        setToast({
          header: "Số điện thoại không tồn tại trên hệ thống",
          content: "",
          bg: "danger",
          textColor: "#fff",
        });
  ...
  const handleSubmit = async (event) => {
    ...
    if (handleValidation()) {
      const { matKhau,soDienThoai } = values;
      const objectDoiMatKhau = {
        soDienThoai:'+84'+soDienThoai,
        matKhauMoi:matKhau
      }
     console.log('Mật khẩu mới :',objectDoiMatKhau);
      const { data } = await axios.post(forgotPassWord, objectDoiMatKhau, {}, {
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
    ...
    console.log("Response forGotPassword :", data);
    if(data){
      setToast({
  ...
  console.log('Số điện thoại :',values.soDienThoai);
  return (
    <>
      <Container>
                    ...
                    <div className="form-container">
                      <Form onSubmit={(e) => handleSendCode(e)}>
                        ...
                        <div className="btn-container">
                          ...
                          {/* <Button variant="success" onClick={()=>{setStep("VERIFY_SUCCESS")}}>
                            Test Doi Mat Khau
                          </Button> */}
                        </div>
            ...
            <div className="wrapper">
              <div className="grid-container">
                <div className="item-grid-login">
                  <div className="box-login">
                    <div className="header_login">
                      <span className="title_header">Đổi mật khẩu mới </span>
